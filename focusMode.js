...
/* -----------------------------------------
   12. PATTERN ENGINE
   Shows and explains sequence patterns that
   exist on VISIBLE candles only (allCandles +
   revealedSoFar). Future candles are excluded.
----------------------------------------- */
 
// Maps seq_* column names to human-readable labels and explanations
const PATTERN_META = {
    seq_compression: {
        label: "Compression",
        explain: "A compression zone is a series of candles where the market's range is contracting or inside bars are stacking up. Think of it as the market coiling a spring — both buyers and sellers are waiting. The longer it compresses, the more energy is building for the eventual breakout. When price finally escapes, the move is often fast and decisive."
    },
    seq_momentum_burst: {
        label: "Momentum Burst",
        explain: "A momentum burst is a sequence of consecutive bullish candles, each stronger than normal and each breaking above the previous high. This is the market accelerating — buyers are not just buying, they are buying at higher and higher prices without hesitation. Momentum bursts can continue for several candles, but they also exhaust themselves, so watch for slowing candle bodies."
    },
    seq_failed_breakout: {
        label: "Failed Breakout",
        explain: "A failed breakout starts with a candle that breaks above a previous high, raising expectations of continuation. But instead of following through, the next candle shows failure — and then a bearish reversal confirms it. This pattern is powerful because it traps buyers who entered on the breakout. Their forced exits accelerate the move downward."
    },
    seq_failed_breakdown: {
        label: "Failed Breakdown",
        explain: "A failed breakdown is the mirror of a failed breakout. Price breaks below a previous low, but buyers step in before the close and push it back. The subsequent bullish candle confirms the failure. Traders who shorted the breakdown are now trapped, and their covering adds fuel to the recovery."
    },
    seq_spring: {
        label: "Spring",
        explain: "A spring is a high-conviction failed breakdown. Not only does price recover after breaking the low — it recovers with a strong bullish candle, showing that buyers were waiting precisely at that level. The name comes from Wyckoff analysis: the market springs off a floor, shaking out weak sellers before launching upward. Springs are considered one of the highest-probability reversal signals in price action."
    },
    seq_engulfing_flip: {
        label: "Engulfing Flip",
        explain: "An engulfing flip is a single strong candle that engulfs the previous candle's body and closes in the opposite direction. When it is strong, it means one side completely overpowered the other in a single session. An engulfing flip at the end of a trend is a potential reversal signal; in the middle of a trend, it can mark an acceleration point."
    },
};
 
/**
 * Returns the set of unique pattern labels present on visible candles only.
 * Visible = allCandles + revealedSoFar (never futureCandles).
 */
function getVisiblePatterns() {
    ...
    visible.forEach(candle => {
        Object.keys(PATTERN_META).forEach(key => {
            if (candle[key] === 1) found.add(key);
        });
    });
...
/**
 * Toggle the pattern tag panel.
 * Shows a pill for each unique pattern on visible candles.
 */
function togglePatterns() {
    ...
    const found = getVisiblePatterns();
 
    if (found.size === 0) {
        panel.innerHTML = '<span class="pattern-none">No sequences detected on visible candles yet.</span>';
    } else {
        panel.innerHTML = Array.from(found)
            .map(key => `<span class="pattern-tag">${PATTERN_META[key].label}</span>`)
            .join('');
    }
...
/**
 * Toggle the pattern explanation panel.
 * Shows a plain-English explanation for each pattern found on visible candles.
 */
function togglePatternExplain() {
    ...
    if (found.size === 0) {
        ...
        panel.innerHTML = Array.from(found).map(key => {
            const meta = PATTERN_META[key];
            return `<div class="pattern-explain-item">
                        <div class="pattern-explain-label">${meta.label}</div>
